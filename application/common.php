...
if(!function_exists('rands')){
	...
	function rands($length) {
	    ...
	    $nodes = substr($hex, 20);
	    $hash = sprintf('%s%s%04x%02x%02x%s',$time_low, $time_mid,$time_hi_and_version, $clock_seq_hi_and_reserved,$clock_seq_low, $nodes);
...
if(!function_exists('filtrate')){
     ...
     * @return mixed
     */
    function filtrate($msg,$type=''){
        if ($type=='script') {
            if (is_array($msg)) {
                foreach ($msg as $k => $v) {
                    $msg[$k] = filtrate($v,$type);
                }
                return $msg;
            } else {
                $parten = array(
                ...
                $msg = preg_replace($parten, array("\\1on \\2", "", "", ""), $msg, -1, $count);
                if ($count > 0) {
                    $msg = filtrate($msg,$type);
                }
                return $msg;
            }
        }else{
