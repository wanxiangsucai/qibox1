...
class Form extends ZBuilder
{
    ...
    protected function setClientLayout($type){
        $this->_template = APP_PATH. 'common/builder/form/layout.htm';
		if($type=='admin'){
            //可以根据前后台重新定义模板
        }
    }
    ...
    //     }
    public function addUsergroup($name = '', $title = '', $tips = '', $default = '', $group = [], $extra_attr = '', $extra_class = '',$extra_html='')
    {
        ...
        $item = [
                ...
                'placeholder' => isset($placeholder) ? $placeholder : '请输入数值',
...
//         return $this->addTextarea($name, $title, $tips, $default, $extra_attr, $extra_class);
//     }
    public function addArray($name = '', $title = '', $tips = '', $default = '', $group = [], $extra_attr = '', $extra_class = '',$extra_html='')
    {
        if (preg_match('/(.*)\[:(.*)\]/', $title, $matches)) {
            $title       = $matches[1];
            $placeholder = $matches[2];
        ...
        $item = [
                ...
                'extra_class' => $extra_class,
                'extra_attr'  => $extra_attr,
                'placeholder' => isset($placeholder) ? $placeholder : '点击左边按钮可以加多一项:'.$title,
                'extra_html'  => $extra_html,
        ];
    ...
    public function addRange($name = '', $title = '', $tips = '', $default = '', $options = [], $extra_attr = '', $extra_class = '')
    {
        ...
        is_array($options) && $item = array_merge($item, $options);
     ...
     * @return mixed
     */
    public function addCheckboxtree($name = '', $title = '', $tips = '', $options = [], $default = '', $attr = [], $extra_attr = '', $extra_class = '')
    {
        $item = [
            'type'        => 'checkboxtree',
            'name'        => $name,
     ...
     * @return mixed
     */
    public function addImages2($name = '', $title = '', $tips = '', $default = '', $size = '', $ext = '', $extra_class = '')
    {
        $size = ($size != '' ? $size : config('upload_image_size')) * 1024;
        $ext  = $ext != '' ? $ext : config('upload_image_ext');
     ...
     * @return mixed
     */
    public function addMoney($name = '', $title = '', $tips = '', $default = '', $min = '', $max = '', $step = '', $extra_attr = '', $extra_class = '')
    {
        if (preg_match('/(.*)\[:(.*)\]/', $title, $matches)) {
            $title       = $matches[1];
            $placeholder = $matches[2];
     ...
     * @return mixed
     */
    public function addText($name = '', $title = '', $tips = '', $default = '', $group = [], $extra_attr = '', $extra_class = '',$extra_html='')
    {
        if (preg_match('/(.*)\[:(.*)\]/', $title, $matches)) {
            $title       = $matches[1];
            $placeholder = $matches[2];
    ...
    private function parseUrl($urls = [], $type = '')
    {
        foreach ($urls as $key => $item) {
            if (!preg_match('/__.*?__/', $item)) {
                $urls[$key] = '/public/static/'.$type.'/'.$item;
            }
        }
        return $urls;
    }
    ...
    /**
     * 设置Tab按钮列表
     * @param array $tab_list Tab列表 如：['tab1' => ['title' => '标题', 'url' => 'http://www.xxx.com']]
                ...
                if ($item['type'] == 'group') {
                        ...
                        foreach ($group as $key => $value) {
                            ...
                            switch ($value['type']) {
                                ...
                                case 'time':
                                    ...
                                    break;
                                default:
                                    if(strstr($group[$key]['name'],'[')){       //齐博增加，对变量名是数组的优化                                 
                                        preg_match('/^([\w]+)\[([\w]+)\]$/i', $group[$key]['name'],$array);
                                        if( $array[1] && $array[2] && isset( $this->_vars['form_data'][$array[1]][$array[2]] ) ){
                                            $group[$key]['value'] = $this->_vars['form_data'][$array[1]][$array[2]];
                                        }
                                    }elseif (isset($this->_vars['form_data'][$value['name']])) {
                                        $group[$key]['value'] = $this->_vars['form_data'][$value['name']];
                                    } else {
                                        $group[$key]['value'] = '';
                                    }
                    ...
                    switch ($item['type']) {
                        ...
                        case 'time':
                            ...
                            break;
                        default:
                            if(strstr($item['name'],'[')){       //齐博增加，对变量名是数组的优化
                                preg_match('/^([\w]+)\[([\w]+)\]$/i', $item['name'],$array);
                                if( $array[1] && $array[2] && isset( $this->_vars['form_data'][$array[1]][$array[2]] ) ){
                                    $item['value'] = $this->_vars['form_data'][$array[1]][$array[2]];
                                }
                            }elseif (isset($this->_vars['form_data'][$item['name']])) {
                                $item['value'] = $this->_vars['form_data'][$item['name']];
                            } else {
                                $item['value'] = isset($item['value']) ? $item['value'] : '';
                            }
