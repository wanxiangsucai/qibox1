...
class Table extends ZBuilder
{
     ...
     */
    protected $_replace_right_buttons = [];

    /**
     * @var bool 有分页数据
     */
    protected $_has_pages = true;

    /**
     * @var array 存储字段筛选选项
     */
    protected $_filter_options = [];

    /***
     * @var array 存储字段筛选列表
     */
    protected $_filter_list = [];

    /**
     * @var array 存储字段筛选类型
     */
    protected $_filter_type = [];
     ...
     */
    protected $_select_list_default = [];
    ...
    protected function setClientLayout($type){
		$this->_template   = APP_PATH. 'common/builder/table/layout.htm';
        if($type=='admin'){
            //可以根据前后台重新定义模板
        }
    }
    ...
    public function addFilterList($field = '', $list = [], $default = '', $type = 'checkbox')
    {
        if ($field != '' && !empty($list)) {
            ...
            $this->_filter_list[$field] = ('_filter_list_'.$this->_module.'_'.$this->_controller.'_'.$this->_action.'_'.$field);
    ...
    private function getDefaultUrl($type = '', $params = [])
    {
        ...
        if($menu['url_type'] == 'module_home'){
            $_url = home_url($url, $params);            
        }else{
            if($this->_plugin_name!=''){
                $_url = purl(end(explode('/',$url)), $params);
            }else{
                $_url = url($url, $params);
            }
        }
    ...
    public function addTopButton($type = '', $attribute = [], $pop = false)
    {
		...
		/*
        if (session('user_db.groupid') != 1 && substr($btn_attribute['href'], 0, 4) != 'http' && $btn_attribute['href'] != 'javascript:history.back(-1);') {
    ...
    private function checkButtonAuth($btn_attribute = [])
    {
       ...
       // return Group::checkAdminPower($url_value, true);
    }
    ...
    public function addRightButton($type = '', $attribute = [], $pop = false)
    {
        ...
        // 判断当前用户是否有权限，没有权限则不生成按钮
		/*
        if (session('user_db.groupid') != 1 && substr($btn_attribute['href'], 0, 4) != 'http') {
            if ($this->checkButtonAuth($btn_attribute) === false) {
                return $this;
            }
        }*/
    ...
    public function setRowList($row_list = null)
    {
        ...
        if (empty($this->_vars['row_list'])) {
            ...
            if (isset($params['page']) && ENTRANCE!='index') {  //齐博修改
                unset($params['page']);
    ...
    /**
     * 设置Tab按钮列表
     * @param array $tab_list Tab列表  ['title' => '标题', 'url' => 'http://www.xxxx.com']
                ...
                foreach ($this->_vars['columns'] as $column) {
                    ...
                    switch ($column['type']) {
                        ...
                        case 'img_url': // 外链图片
                            ...
                            break;
                        case 'picture': // 单张图片
                            $row[$column['name'].'__'.$column['type']] = '<div class="js-gallery"><a href="'.get_file_path($row[$column['name']]).'" class="img-link" title="'.($row[$column['name']]).'"><img class="image" src="'.get_file_path($row[$column['name']]).'"></a></div>';
                            break;
                        case 'pictures': // 多张图片
                            if ($row[$column['name']] === '') {
                                ...
                                foreach ($list_img as $key => $img) {
                                    ...
                                    $imgs .= ' <a href="'.get_file_path($img).'" class="img-link" title="'.($img).'"><img class="image" src="'.get_file_path($img).'"></a>';
                                }
                        ...
                        case 'select2': // tag编辑(有BUG)
                            if ($column['default']) {
                               // $row[$column['name'].'__'.$column['type']] = $row[$column['name']];
                                $row[$column['name'].'__'.$column['type']] = $column['default'][$row[$column['name']]] ;
                           }
                            break;
