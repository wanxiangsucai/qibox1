...
class Qb extends TagLib{
    ...
    protected $tags   =  [
            ...
            'form'  => ['attr' => 'name,info,mid,field,mod,f_array,val', 'close' => 1],     //field 过滤循环不显示的字段,多个用,号隔开,f_array是程序中自由定义的字段
    ...
    public function tagForm($tag,$content){         //$content 的内容就是 <!--###break###--!>
        ...
        $parse .= ' QB--><?php endif; ?>';
        $parse .= '<?php '."fun('label@run_form_label','$name',[$_farray'val'=>'$val','mid'=>$_mid,'info'=>\$$info,'field'=>'$field','mod'=>'$mod','dirname'=>__FILE__,]);".' ?>';
        return $parse;
    }
