...
trait Market
{
    ...
    protected function getapp($id=0,$type='m'){
        if (!table_field('config','is_open')) {
            into_sql("ALTER TABLE  `qb_config` ADD  `is_open` TINYINT NOT NULL COMMENT  '是否公开给接口调用，密钥就不要公开';
...
//                 copy_dir(RUNTIME_PATH."model/template",TEMPLATE_PATH,true);
    ...
    protected function replace_class_name($oldkey,$newkey,$type='m'){
        ...
        foreach ($file_array AS $file){
            $str = str_replace([" $basename\\$oldkey\\"," $basename\\$oldkey;","$basename\\$oldkey::"] , [" $basename\\$newkey\\"," $basename\\$newkey;","$basename\\$newkey::"] , read_file($file));
