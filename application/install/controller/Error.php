    ...
    private function step4()
    {
        if ($this->request->isPost()) {
            if (!is_writable(APP_PATH.'database.php')) {
                return $this->error('[application/database.php]无读写权限！');
            }
            $data = input('post.');
            $data['type'] = 'mysql';
            $rule = [
...
//             }
    ...
    private function step5()
    {
        ...
        $rule = [
            ...
            'password|管理员密码' => 'require|length:5,20',
        ];
        $validate = $this->validate(['username' => $username, 'password' => $password], $rule);
        if (true !== $validate) {
            return $this->error($validate);
        }
        // 导入系统初始数据库结构
        ...
        Db::name("memberdata")->where(['uid'=>1])->update($array);
        //站点密匙
        $auth = input('post.mymd5')?:substr(md5(time().get_ip()),0,rand(25,32));
        ...
        return $this->success('系统安装成功，欢迎您使用齐博系统', $root_dir.'admin.php');
    }
    ...
    {
        $items = [
            'os'      => ['操作系统', '不限制', 'Windows/类Unix', PHP_OS, 'ok'],
...
INFO;
        write_file(APP_PATH.'database.php', $code);
        // 判断写入是否成功
        $config = include APP_PATH.'database.php';
