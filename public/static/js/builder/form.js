...
jQuery(document).ready(function() {
	...
	var add_cutimg = function(e){
		e.find('.js_cutimg').each(function () {
			var $self = $(this);
			$self.on('click',function(){
				var pic = $self.data('url');
				var opt = $self.data('options');
				if(opt=='' && $('#cut_image_size').length>0){
					opt = $('#cut_image_size').val();
				}
				if(opt=='' && $('#pic_width').length>0 && $('#pic_height').length>0 ){
					opt = $('#pic_width').val()+':'+$('#pic_height').val();
				}
				layer.open({
					type: 2,
					title: '截图',
					area: ["1000px", "800px"],
					scrollbar: false,
					content: '/index.php/index/image/cutimg.html?picurl='+pic+'&opt='+opt
				});
			});
		});
	}
	...
	$('.list_usergroup').each(function () {
		...
		var default_value = base.find('textarea').val();
		obj = JSON.parse(default_value);
		for(var item in obj){
    ...
    $('.js-upload-image,.js-upload-images').each(function () {
        ...
        uploader.on( 'fileQueued', function( file ) {
            var $li = $(
                    ...
                    '<i class="fa fa-times-circle remove-picture"></i><a class="js_cutimg" data-options="" data-url="">截图</a>' +
        ...
        uploader.on( 'uploadSuccess', function( file, response ) {
			...
			$li.find('.js_cutimg').attr("data-url",response.path);
			add_cutimg( $li );	//添加截图事件
	    ...
	    // 图片上传带附注
    $('.js-upload-images2').each(function () {
        var $input_file       = $(this).find('input.pic');
        var $input_file_name  = $input_file.attr('name');
        // 是否多图片上传
        var $multiple         = $input_file.data('multiple');
        // 允许上传的后缀
        ...
        uploader.on( 'fileQueued', function( file ) {
            var $li = $(
                    ...
                    '<div class="file-state img-state"><div class="bg-info">正在读取...</div>' +
                    '</div>'+
					'<input placeholder="介绍：可为空" style="width:120px;margin:5px 0 5px 0;border:1px solid #eee;color:#888;" class="about" name="images2[' + $input_file_name + ']['+ file.id + '][title]" value="' + file.name + '"><br>'+
					'<input placeholder="网址：可为空" style="width:120px;border:1px solid #eee;color:#888;" class="about" name="images2[' + $input_file_name + ']['+ file.id + '][url]" value=""><br>'+
				    '<input type="hidden" class="iurl" name="images2[' + $input_file_name + ']['+ file.id + '][picurl]" value="">'+
					'<input type="button" class="js_cutimg" data-options="" data-url="" value="截图">'+
					'</div>'
                ),
                $img = $li.find('img');
        ...
        uploader.on( 'uploadSuccess', function( file, response ) {
            ...
            if (response.code) {
                if ($multiple) {
					...
					$li.find('.iurl').val(response.id);
                    $li.find('.remove-picture').attr('data-id', response.id);
					makepicurl();	//把链接放入隐藏变量
					input_change();	//介绍或链接内容变动
					$li.find('.js_cutimg').attr("data-url",response.path);
					add_cutimg( $li );	//添加截图事件
                } else {
                    $input_file.val(response.id);
		...
		//读取数据
		var makepicurl = function(){
				var selectData = [];
				$file_list.find('.thumbnail').each(
					function(i,v){
						ipt = $(this).find('input');
						selectData.push({"title":ipt.eq(0).val(), "url":ipt.eq(1).val(), "picurl":ipt.eq(2).val()});
					}
				);
				$input_file.val(JSON.stringify(selectData));
				//alert(JSON.stringify(selectData));//$.parseJSON('{"aspectRatio":"400/300"}')
		};
		//input失去焦点后,重新处理数据
		var input_change = function(){
			$file_list.find('.thumbnail input').each(
				function(){
					$(this).blur(
						function(){
							makepicurl();
