...
class Template
{
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . (defined('TPL_CACHE_PRE') ? TPL_CACHE_PRE : '') . md5($this->config['layout_name'] . $template) . '.' . ltrim($this->config['cache_suffix'], '.');
    ...
    public function display($content, $vars = [], $config = [])
    {
        ...
        $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . (defined('TPL_CACHE_PRE') ? TPL_CACHE_PRE : '') . md5($content) . '.' . ltrim($this->config['cache_suffix'], '.');
        if (!$this->checkCache($cacheFile)) {
            // 缓存无效 模板编译
            $this->compiler($content, $cacheFile);
        }
   ...
   private function parseTemplateFile($template){
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if ($this->config['view_base']) {
				...
				//齐博修改,之前存在BUG,不支持自定义的多级模板目录
				if(ENTRANCE === 'admin'){
					if($module=='index'||$module=='member'){
						$path = APP_PATH . $module . DS . 'view' . DS . 'default' . DS;
					}else{
						$path = isset($module) ? APP_PATH . $module . DS . basename($this->config['view_path']) . DS : $this->config['view_path'];
					}
				}else{
					if($module=='index'||$module=='member'){
						//index与member模块特别一点,view下面不使用index、admin、member这三个目录,所以不能使用$this->config['view_path']
						if($module=='member'){
							$view_path = APP_PATH . '/view/'.$this->config['member_style'].'/';
							$this->config['member_style']!='default' && $default_view_path = APP_PATH . '/view/default/';
						}else{
							$view_path = APP_PATH . '/view/'.$this->config['style'].'/';
							$this->config['style']!='default' && $default_view_path = APP_PATH . '/view/default/';
						}						
					}else{
						$view_path = $this->config['view_path'];
						$this->config['style']!='default' && $default_view_path = $this->config['default_view_path'];
					}
					$path = isset($module) ? APP_PATH . $module . strstr(str_replace(APP_PATH,'',$view_path),'/') : $this->config['view_path'];
					$this->config['style']!='default' && $default_path = isset($module) ? APP_PATH . $module . strstr(str_replace(APP_PATH,'',$default_view_path),'/') : $this->config['default_view_path'];
				}
            }
			$_template = $template;
        ...
        if (is_file($template)) {
            ...
            throw new TemplateNotFoundException('template not exists:' .$tpl.'<br>'. $template, $template);
        }
    }
